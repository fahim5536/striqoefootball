...
// Replace the return block with the premium layout structure and classes
                ...
                return (
                  <div key={article.id} className="news-card" onClick={() => setSelectedArticle(article)} style={{ cursor: 'pointer' }}>
                    ...
                    <div className="news-content">
                      ...
                      <p className="news-excerpt">{article.excerpt || article.content.substring(0, 100) + '...'}</p>
            ...
            <div style={{ padding: '40px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                ...
                <span style={{ color: '#94a3b8', fontSize: '14px', fontWeight: 600 }}>
                  {new Date(selectedArticle.createdAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }).toUpperCase()}
                </span>
              </div>
...
content = content.replace(/return \(\s*<>\s*<section className="news-section">[\s\S]*?\);\s*\}/, replacement + "\n}");
